...
##     the hand skeleton, and uses this position to move a CapsuleShape3D bone
##     CharacterBody3D.
##
...
func _ready():
	...
	if physics_hand:
		physics_hand.connect("hand_scale_changed",Callable(self,"_on_hand_scale_changed"))
...
func _move_bone(delta: float) -> void:
	...
	_physics_bone.set_velocity(bone_vel)
	_physics_bone.set_up_direction(Vector3.UP)
	_physics_bone.move_and_slide()
	_physics_bone.velocity
