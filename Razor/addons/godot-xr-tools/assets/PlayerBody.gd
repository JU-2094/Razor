...
##
##     The PlayerBody can detect when the player is in the air, checked the ground,
##     or checked a steep slope.
##
...
@export var player_radius := 0.4 :
	get:
		return player_radius # TODOConverter40 Non existent get function 
	set(mod_value):
...
## Force of gravity checked the player
...
@export (Resource) var physics = null :
	get:
		return physics # TODOConverter40 Non existent get function 
	set(mod_value):
		mod_value  # TODOConverter40 Copy here content of set_physics

# Set our collision layer
@export (int, LAYERS_3D_PHYSICS) var collision_layer = 1 << 19 :
	get:
...
func _ready():
	...
	_movement_providers.sort_custom(Callable(SortProviderByOrder,"sort_by_order"))
...
func _physics_process(delta: float):
	...
	# Decrement the jump cool-down checked each physics update
	if _jump_cooldown:
		_jump_cooldown -= 1
...
func request_jump(skip_jump_velocity := false):
	...
	# Skip if not checked ground
	if !on_ground:
		return

	# Skip if jump disabled checked this ground
...
# Perform a move_and_slide checked the kinematic node
set_velocity(var velocity: Vector3)
move_and_slide()()
func velocity -> Vector3:
	kinematic_node.set_velocity(velocity)
	kinematic_node.set_up_direction(Vector3.UP)
	kinematic_node.set_floor_stop_on_slope_enabled(false)
	kinematic_node.set_max_slides(4)
	kinematic_node.set_floor_max_angle(0.785398)
	# TODOConverter40 infinite_inertia were removed in Godot 4.0 - previous value `push_rigid_bodies`
...
func _update_body_under_camera():
	# Calculate the player height based checked the camera position in the origin and the calibration
	...
	# Center the kinematic body checked the ground under the camera
...
func _update_ground_information(delta: float):
	...
	# Detect if we're sliding checked a wall
...
func _apply_velocity_and_control(delta: float):
	...
	# If the player is checked the ground then give them control
	if on_ground:
		...
		if abs(ground_control_velocity.x) > 0.1 or abs(ground_control_velocity.y) > 0.1:
			...
			# Apply control velocity to horizontal velocity based checked traction
	...
	set_velocity(local_velocity + ground_velocity)
	move_and_slide()
	velocity = velocity
...
func _get_configuration_warnings():
	...
	if !test_camera_node:
		return "Unable to find ARVR Camera3D node"
...
static func get_player_body(node: Node, path: NodePath = "") -> PlayerBody:
