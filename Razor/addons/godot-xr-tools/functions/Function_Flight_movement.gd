...
# Node references
@onready var _camera: XRCamera3D = ARVRHelpers.get_arvr_camera(self)
@onready var _left_controller: XRController3D = ARVRHelpers.get_left_controller(self)
@onready var _right_controller: XRController3D = ARVRHelpers.get_right_controller(self)
...
func physics_movement(delta: float, player_body: PlayerBody, disabled: bool):
	...
	if exclusive:
		player_body.set_velocity(flight_velocity)
